 ...
 * This class needs the ji2c.c C library to communicate on the bus. This library is given together with the jarmadeus package. <br>
...
public class I2CTalker {
	...
	/**
	 * @param addr  Sensor's address on the i2c bus. This address must follow the lm_sensor i2c address format, i.e. it only takes into account the 7 highest bits of the i2c adress. For example,  if the i2c address is 0xE0, then the lm_sensor i2c address is 0x70. This way of addressing may be justified by the fact that the lowest bit of i2c address is used by i2c bus to specify a write or a read order.  
	 * @param reg  address of the sensor's register to read 
	...
	/**
	 * @param addr  Sensor's address on the i2c bus. This address must follow the lm_sensor i2c address format, i.e. it only takes into account the 7 highest bits of the i2c adress. For example,  if the i2c address is 0xE0, then the lm_sensor i2c address is 0x70. This way of addressing may be justified by the fact that the lowest bit of i2c address is used by i2c bus to specify a write or a read order. 
	 * @param reg   address of the sensor's register to write 
	 * @param data  data to be written in the sensor's register
